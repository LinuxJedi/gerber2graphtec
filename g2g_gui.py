...
def Send_to_Cutter():
    ...
    try:
      ...
      tkMessageBox.showerror("G2G_GUI ERROR", "There was an error sending the Graphtec file to the plotter")
      return
...
def get_input_filename():
    input_filename=tkFileDialog.askopenfilename(title='Select paste mask Gerber file', filetypes=[('Gerber File', ('*.g*', '*.G*')),("All files", "*.*")] )
    if input_filename:
